...
@register.filter
def create_word_type_anchor(entry, title):
	"""
		Short hand for creating anchor for given referenced entry with given title
		<a href="/vocabularies/lookup?uri={{ ref.object.uri }}" title="Show {{ ref.relation }}">{{ ref.object.get_label }}</a>
		"""
	if 'http://www.openvocabulary.info/ontology/'.startswith(BASE_URL_PATH):
		template = "<a href='http://www.openvocabulary.info/ontology/I%s' title='%s'>%s</a>"
	else:
		template = "<a href='/vocabularies/lookup?uri=http://www.openvocabulary.info/ontology/I%s' title='%s'>%s</a>"

	return mark_safe(
		template % (_escape_apostrophe(entry), title, _escape_apostrophe(entry)))
